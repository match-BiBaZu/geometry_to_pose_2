...
for si = 1:numStable
    ...
    patch(ax1, floorW(:,1), floorW(:,2), floorW(:,3), ...
        [0.72 0.90 0.72], 'FaceAlpha', 0.25, ...
        'EdgeColor', [0.20 0.55 0.20], 'LineWidth', 0.8);
    patch(ax1, wallW(:,1), wallW(:,2), wallW(:,3), ...
        [0.72 0.90 0.72], 'FaceAlpha', 0.25, ...
        'EdgeColor', [0.20 0.55 0.20], 'LineWidth', 0.8);

    trisurf(partFaces, vertsWorld(:,1), vertsWorld(:,2), vertsWorld(:,3), ...
        'Parent', ax1, ...
        'FaceColor', [1.00 0.92 0.23], 'FaceAlpha', 0.50, ...
        'EdgeColor', [0.40 0.35 0.00], 'EdgeAlpha', 0.20);

    scatter3(ax1, vertsWorld(floorContactIdx,1), ...
        vertsWorld(floorContactIdx,2), vertsWorld(floorContactIdx,3), ...
        30, [0.10 0.60 0.10], 'filled', 'MarkerFaceAlpha', 0.9);
    scatter3(ax1, vertsWorld(wallContactIdx,1), ...
        vertsWorld(wallContactIdx,2), vertsWorld(wallContactIdx,3), ...
        30, [0.10 0.60 0.10], 'filled', 'MarkerFaceAlpha', 0.9);

    scatter3(ax1, centWorld(1), centWorld(2), centWorld(3), 70, 'k', 'filled');

    if ~isempty(ov_vertsWorld)
        trisurf(partFaces, ...
            ov_vertsWorld(:,1), ov_vertsWorld(:,2), ov_vertsWorld(:,3), ...
            'Parent', ax1, ...
            'FaceColor', ov_faceCol, 'FaceAlpha', 0.45, ...
            'EdgeColor', ov_edgeCol, 'EdgeAlpha', 0.20);
        scatter3(ax1, ov_centWorld(1), ov_centWorld(2), ov_centWorld(3), ...
    ...
    if hasWallGeo
        ...
        scatter3(ax1, pivW_w(1), pivW_w(2), pivW_w(3), ...
            100, wallCol, 'd', 'filled');
        plot3(ax1, [pivW_w(1), armEndW_w(1)], ...
                   [pivW_w(2), armEndW_w(2)], ...
                   [pivW_w(3), armEndW_w(3)], ...
            '-', 'Color', wallCol, 'LineWidth', wallLineW);

        pivOnWall_c    = geo.pivotWall_c;
        pivOnWall_c(2) = 0;
        pivOnWall_w    = (Rchute * pivOnWall_c(:))';
        plot3(ax1, [pivW_w(1), pivOnWall_w(1)], ...
                   [pivW_w(2), pivOnWall_w(2)], ...
                   [pivW_w(3), pivOnWall_w(3)], ...
            '-.', 'Color', wallCol, 'LineWidth', wallLineW);
        plot3(ax1, [axWall_A(1), axWall_B(1)], ...
                   [axWall_A(2), axWall_B(2)], ...
                   [axWall_A(3), axWall_B(3)], ...
            '--', 'Color', [0.85 0.20 0.85], 'LineWidth', 1.8);

        midW = 0.5*(pivW_w + armEndW_w);
        text(ax1, midW(1), midW(2), midW(3), ...
            sprintf('  l_A=%.3f\n  l_w=%.3f\n  r=%.3f', ...
                geo.l_A_wall, geo.l_w, ratioWall(si)), ...
            'Color', wallCol, 'FontSize', 7, 'FontWeight', 'bold', ...
            'Interpreter', 'none');
    end
    ...
    if hasFloorGeo
        ...
        scatter3(ax1, pivF_w(1), pivF_w(2), pivF_w(3), ...
            100, floorCol, 'd', 'filled');
        plot3(ax1, [pivF_w(1), armEndF_w(1)], ...
                   [pivF_w(2), armEndF_w(2)], ...
                   [pivF_w(3), armEndF_w(3)], ...
            '-', 'Color', floorCol, 'LineWidth', floorLineW);

        pivOnFloor_c    = geo.pivotFloor_c;
        pivOnFloor_c(3) = 0;
        pivOnFloor_w    = (Rchute * pivOnFloor_c(:))';
        plot3(ax1, [pivF_w(1), pivOnFloor_w(1)], ...
                   [pivF_w(2), pivOnFloor_w(2)], ...
                   [pivF_w(3), pivOnFloor_w(3)], ...
            '-.', 'Color', floorCol, 'LineWidth', floorLineW);
        plot3(ax1, [axFloor_A(1), axFloor_B(1)], ...
                   [axFloor_A(2), axFloor_B(2)], ...
                   [axFloor_A(3), axFloor_B(3)], ...
            '--', 'Color', [0.20 0.70 0.20], 'LineWidth', 1.8);

        midF = 0.5*(pivF_w + armEndF_w);
        text(ax1, midF(1), midF(2), midF(3), ...
            sprintf('  l_A=%.3f\n  l_f=%.3f\n  r=%.3f', ...
                geo.l_A_floor, geo.l_f, ratioFloor(si)), ...
            'Color', floorCol, 'FontSize', 7, 'FontWeight', 'bold', ...
            'Interpreter', 'none');
    end
    ...
    annotation(hFig, 'textbox', ...
        'Units', 'normalized', ...
        'Position', [0.27, 0.68, 0.24, 0.24], ...
        'String', quatStr, ...
        'FontSize', 7.5, ...
        'FontName', 'Courier', ...
        'BackgroundColor', [0.94 0.97 1.00], ...
        'EdgeColor', [0.30 0.45 0.70], ...
        'LineWidth', 0.8, ...
        'Margin', 4, ...
        'FitBoxToText', 'on', ...
        'Interpreter', 'none');
    ...
    zoomPts = vertsWorld;
    if ~isempty(ov_vertsWorld), zoomPts = [zoomPts; ov_vertsWorld]; end
    ...
    trisurf(partFaces, vertsWorld(:,1), vertsWorld(:,2), vertsWorld(:,3), ...
        'Parent', ax2, ...
        'FaceColor', [0.8 0.8 0.8], 'FaceAlpha', 0.12, ...
        'EdgeColor', [0.7 0.7 0.7], 'EdgeAlpha', 0.08);
    if ~isempty(nonContactSet)
        scatter3(ax2, vertsWorld(nonContactSet,1), vertsWorld(nonContactSet,2), ...
            vertsWorld(nonContactSet,3), 8, [0.65 0.65 0.65], 'filled', 'MarkerFaceAlpha', 0.4);
    end
    if ~isempty(floorOnlySet)
        scatter3(ax2, vertsWorld(floorOnlySet,1), vertsWorld(floorOnlySet,2), ...
            vertsWorld(floorOnlySet,3), 16, [0.15 0.45 0.90], 'filled');
    end
    if ~isempty(wallOnlySet)
        scatter3(ax2, vertsWorld(wallOnlySet,1), vertsWorld(wallOnlySet,2), ...
            vertsWorld(wallOnlySet,3), 16, [0.85 0.15 0.15], 'filled');
    end
    if ~isempty(bothSet)
        scatter3(ax2, vertsWorld(bothSet,1), vertsWorld(bothSet,2), ...
            vertsWorld(bothSet,3), 16, [0.80 0.10 0.80], 'filled');
    end
    for kk = 1:size(supportVerts,1)
        p_ = supportVerts(kk,:); pp = p_; pp(3) = planeZ;
        plot3(ax2, [p_(1),pp(1)], [p_(2),pp(2)], [p_(3),pp(3)], ...
            ':', 'Color', [0.6 0.6 0.6 0.5], 'LineWidth', 0.8);
    end
    if ~isempty(hullProj3D)
        patch(ax2, hullProj3D(:,1), hullProj3D(:,2), hullProj3D(:,3), ...
            [0.2 0.6 1.0], 'FaceAlpha', 0.30, ...
            'EdgeColor', [0.1 0.35 0.80], 'LineWidth', 1.5);
        for kk = 1:size(hullProj3D,1)
            plot3(ax2, [centWorld(1), hullProj3D(kk,1)], ...
                [centWorld(2), hullProj3D(kk,2)], ...
                [centWorld(3), hullProj3D(kk,3)], ...
                'Color', [0.3 0.3 0.3], 'LineWidth', 1.2);
        end
    end
    scatter3(ax2, supportProj(:,1), supportProj(:,2), supportProj(:,3), ...
        10, [0.1 0.35 0.80], 'filled', 'MarkerFaceAlpha', 0.7);
    scatter3(ax2, centWorld(1), centWorld(2), centWorld(3), 55, 'k', 'filled');
    scatter3(ax2, centProjFloor(1), centProjFloor(2), centProjFloor(3), ...
        55, [0.05 0.60 0.05], 'filled');
    plot3(ax2, [centWorld(1), centProjFloor(1)], ...
        [centWorld(2), centProjFloor(2)], ...
        [centWorld(3), centProjFloor(3)], ...
